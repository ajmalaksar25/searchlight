...
1. The `gsc` MCP server must be connected. Call `auth_status`.
   - Not authenticated → tell them to run `gsc-mcp login` (or `login --setup` for provisioning), or use the `auth_login` tool.
   - For provisioning (creating GA4/verification), `setupMode` must be true (server started with `--setup`) and the token must include the setup scopes (re-login with `--setup`).
